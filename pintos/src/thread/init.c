...
#include "threads/my-edf-test.c"   /* <-- EDF demo test */
#ifdef USERPROG
...
#else
  /* For kernel-only mode, run our EDF demo directly if requested,
     otherwise fall back to the standard test runner. */
  if (!strcmp (task, "my-edf-test"))
    my_edf_test ();
  else
    run_test (task);
#endif
